...
switch($_POST['action']){
	...
	case 'generateStsRebTyp':
		...
		if($stsServerCon){
			...
			$logDate = date("Y-m-d", strtotime("$serverDate", time()));
			$logs = "
			select date,typeId,status from tblLogs where typeId = 1 and date = '{$logDate}' and status = 1
			";
			$qrylogs = mssql_query($logs);
			if(mssql_num_rows($qrylogs) > 0){
			...
			//if(mssql_query($sqlExecSaveARCLastCSV)){
				...
				if($strlength > 0){
					...
					while($myrow=mssql_fetch_array($arrSqlCimCusFin)){
						...
						$xcontentx .= trim($myrow['LOYALTY_CARD_ISSUE_DATE']).","; #11
						...
						$xcontentx .= '"'.trim(date("Y-m-d h:i:s",strtotime($myrow['CARD1_ISSUE_DATE']))).'",'; #24
						$xcontentx .= '"'.trim(date("Y-m-d h:i:s",strtotime($myrow['CARD1_EXPIRE_DATE']))).'",'; #25
						$xcontentx .= '"'.trim(date("Y-m-d h:i:s",strtotime($myrow['CARD2_ISSUE_DATE']))).'",'; #26
						$xcontentx .= trim($myrow['CARD2_EXPIRY_DATE']).","; #27
						$xcontentx .= trim($myrow['ACTIVE']).","; #28
						$xcontentx .= trim($myrow['ARC_DATE']); #29
						$xcontentx .= "\r\n";
					}
					$create = fopen($file_desti.$filename, "x"); //uses fopen to create our file.
					$create2 = fopen($file_desti_arc_server.$filename, "x"); //uses fopen to create our file.
