...
switch($_POST['action']){
	...
	case 'generateCimLoyPoi':
		...
		if($cimsServerCon){
			...
			$filename = "LOYALTY_PTS_".$serverCurrentDate.$fileExt;
	...
	case 'generateCimCusFin':
		...
		if($cimsServerCon){
			...
			$filename = "CUSTOMER_".$serverCurrentDate.$fileExt;
			...
			//if(mssql_query($sqlExecSaveARCLastCSV)){
				...
				if($strlength > 0){
					...
					$xcontentx .= trim("CUSTOMER_CODE, COMPANY_NAME, CUSTOMER_GROUP, 
					GENDER_CODE, HOUSE_HOLD, INCOME, 
					MARITAL_STATUS, LOYALTY_STATUS, BIRTH_DATE, 
					LOYALTY_CARD_NO, LOYALTY_CARD_ISSUE_DATE, FIRST_NAME, 
					LAST_NAME, ADDRESS1, ADDRESS2, LOCATION1, 
					...
					while($myrow=mssql_fetch_array($arrSqlCimCusFin)){
						...
						$xcontentx .= trim($myrow['CUSTOMER_CODE']).",";
						$xcontentx .= trim($myrow['COMPANY_NAME']).",";
						$xcontentx .= trim($myrow['CUSTOMER_GROUP']).",";
						$xcontentx .= trim($myrow['GENDER_CODE']).",";
						$xcontentx .= trim($myrow['HOUSE_HOLD']).",";
						$xcontentx .= trim($myrow['INCOME']).",";
						$xcontentx .= trim($myrow['MARITAL_STATUS']).",";
						$xcontentx .= trim($myrow['LOYALTY_STATUS']).",";
						$xcontentx .= trim($myrow['BIRTH_DATE']).",";
						$xcontentx .= trim($myrow['LOYALTY_CARD_NO']).",";
						$xcontentx .= trim($myrow['LOYALTY_CARD_ISSUE_DATE']).",";
						$xcontentx .= trim($myrow['FIRST_NAME']).",";
						$xcontentx .= trim($myrow['LAST_NAME']).",";
						$xcontentx .= trim($myrow['ADDRESS1']).",";
						$xcontentx .= trim($myrow['ADDRESS2']).",";
						$xcontentx .= trim($myrow['LOCATION1']).",";
						$xcontentx .= trim($myrow['LOCATION2']).",";
						$xcontentx .= trim($myrow['LOCATION3']).",";
						$xcontentx .= trim($myrow['PHONE1']).",";
						$xcontentx .= trim($myrow['PHONE2']).",";
						$xcontentx .= trim($myrow['ACCEPT_CHEQUES']).",";
						$xcontentx .= trim($myrow['CARD_NO1']).",";
						$xcontentx .= trim($myrow['CARD_NO2']).",";
						$xcontentx .= trim($myrow['CARD1_ISSUE_DATE']).",";
						$xcontentx .= trim($myrow['CARD1_EXPIRE_DATE']).",";
						$xcontentx .= trim($myrow['CARD2_ISSUE_DATE']).",";
						$xcontentx .= trim($myrow['CARD2_EXPIRY_DATE']).",";
						$xcontentx .= trim($myrow['ACTIVE']).",";
						$xcontentx .= trim($myrow['ARC_DATE']);
						$xcontentx .= "\r\n";
					}
					$create = fopen($file_desti.$filename, "x"); //uses fopen to create our file.
					fwrite($create, $xcontentx);
		...
		<script type="text/javascript">
			...
			function generateCimCusFin(){
				$.ajax({
					...
					success: function(data){
						$('#btnGenerateCsv').attr('enabled','enabled');
						$(".btn:hover").css({
							"text-decoration": "none",
							"background": "#3498db"
						});
