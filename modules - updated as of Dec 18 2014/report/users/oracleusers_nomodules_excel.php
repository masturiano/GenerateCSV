...
$worksheet->write(2, 4, 'Last Update Date',$headerFormat1);
$worksheet->write(2, 5, 'Last Logon Date',$headerFormat1);
$worksheet->write(2, 6, 'Description',$headerFormat1);
...
$orauserqry = "SELECT fnd_user.USER_ID, fnd_user.USER_NAME, fnd_user.START_DATE, fnd_user.END_DATE, fnd_user.DESCRIPTION,fnd_user.LAST_UPDATE_DATE,fnd_user.LAST_LOGON_DATE
FROM fnd_user 
where fnd_user.user_id >= 1000
GROUP BY fnd_user.USER_ID, fnd_user.USER_NAME, fnd_user.START_DATE, fnd_user.END_DATE, fnd_user.DESCRIPTION,fnd_user.LAST_UPDATE_DATE,fnd_user.LAST_LOGON_DATE
...
foreach($rs as $val)
{
	...
	/*}*/
		...
		$worksheet->write($ctr, 4, $rs->fields['LAST_UPDATE_DATE'],$headerFormat1);
		$worksheet->write($ctr, 5, $rs->fields['LAST_LOGON_DATE'],$headerFormat1);
		$worksheet->write($ctr, 6, $rs->fields['DESCRIPTION'],$headerFormat1);
