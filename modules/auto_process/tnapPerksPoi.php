...
switch($_POST['action']){
	...
	case 'importPerksPoi':
        ...
        if ($checkEmpty == "Empty"){
            ...
            ";
            $truncTableTemp = "
            truncate table tblTnapPerksPointsTemp
            ";
    ...
    select cCardnumber,cCustFullName,iPoints,dLastTranDate,NULL,NULL,lastName from tblTnapPerksPointsTemp
            */
            ...
            if($sql_exec_sql_truncTable = mssql_query($truncTable)){ 
                if($sql_exec_sql_truncTableTemp = mssql_query($truncTableTemp)){ 
    ...
    case 'generateTnapAndPerksPoi':
        ...
        if($cimsServerCon){
            ...
            $tranDate = date("m/d/Y", strtotime("$serverDate - 1", time()));
                ...
                $sqlBcpCheckLen="
                select top 10 * from tblTnapPerksPoints
                ";    
                
                $sqlBcp="
                EXECUTE master.dbo.xp_cmdshell  'bcp \"SELECT * from ExclusivesHO..ViewPerksAndTnapPointsWeb \"  queryout C:\PerksTnap\\$filename -t -c  -Ualp -Palp2015 -S192.168.200.231'
                ";
                
                $arrSqlCustPoiBcp = mssql_query($sqlBcpCheckLen);

                if (file_exists($file_desti.$filename)) {
                    unlink($file_desti.$filename);
                }
                $strlength = strlen($arrSqlCustPoiBcp); //gets the length of our $content string.
                //$xcontentx = "";
                if($strlength > 0){
			...
			function generateTnapAndPerksPoi(){
				$.ajax({
					...
					success: function(data){
                        ...
                        $('.divLogs-TnapPerksCsv').append('Message: Combination of Perks points and Tnap points textfile created.'+'<br/>').css('color','white');
					}		
				})	
			}
				...
				<form class="form-horizontal">
					...
					<div class="dvContainer">
						...
						<div class="divLogs">
                            ...
                            <div class="divLogs-TnapPerksCsv">
                            </div>    
						</div>
