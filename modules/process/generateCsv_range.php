...
switch($_POST['action']){
	...
	case 'generateStsReaReb':
		...
		if($stsServerCon){
			...
			$serverCurrentDateMinOne = date("dmY", strtotime("$serverDate -5 day", time()));
			//$dateFromLY = date("Y-m-d", strtotime("$dtFrom -1 year", time()));
			
			$filename = "REBATES_REA_".$serverCurrentDateMinOne.$fileExt;
			
			//6/24/1987
			$tranDate = date("m/d/Y", strtotime("$serverDate  -5 day", time()));
				...
				<form class="form-horizontal">
					...
					<div class="dvContainer">
						...
						<div class="divLogs">
							...
							<?
							// check open
                            //if (!mssql_connect('promofund-sts','PFSTS','PFSTS')) {
